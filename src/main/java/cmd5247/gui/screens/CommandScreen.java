...
/**
 * Copied from {@link net.minecraft.client.gui.screens.ChatScreen}.
...
public class CommandScreen extends Screen {
  ...
  // private static final int TOOLTIP_MAX_WIDTH = 210; // unused variable ??
  ...
  protected void init() {
    ...
    this.historyPos = CMD.getInstance().commands.getRecentCommands().size();
    this.input = new EditBox(this.minecraft.fontFilterFishy, 4, this.height - 12, this.width - 4, 12, Component.translatable("chat.editBox")) {
      protected MutableComponent createNarrationMessage() {
        return super.createNarrationMessage().append(CommandScreen.this.commandSuggestions.getNarrationMessage());
      }
    };
  ...
  public void moveInHistory(int p_95589_) {
    ...
    int j = CMD.getInstance().commands.getRecentCommands().size();
    i = Mth.clamp(i, 0, j);
    if (i != this.historyPos) {
      if (i == j) {
        ...
        this.input.setValue(CMD.getInstance().commands.getRecentCommands().get(i));
  ...
  public boolean handleChatInput(String p_242400_, boolean p_242161_) {
    ...
    if (p_242400_.isEmpty()) {
      ...
      if (p_242161_) {
        ...
        CMD.getInstance().commands.addRecentCommand(p_242400_);
      }
      ...
      // }
      if (p_242400_.startsWith("$")) {
        CMD.getInstance().commands.performCommand(p_242400_);
      }
